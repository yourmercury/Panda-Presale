...
contract MyToken is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public end;
    uint256 public uniqeContributors;
    uint256 public contribution;
    uint256 public maxPerWallet;
    uint256 private nonce = 1;
    uint256 private platformCut;
    uint256 private platformToken;
    uint256 public dumpProfit;
    ...
    mapping(address => uint256) public contributorValue;
    ...
    modifier presalePeriod() {
        require(
            end > block.timestamp && start < block.timestamp,
            "outside presale period"
        );
        _;
    }
    ...
    function contribute() external payable presalePeriod {
        ...
        require(
            contributor[msg.sender] < maxPerWallet,
            "you have ordered maximally"
        );
    ...
    function dump() external presalePeriod {
        ...
        uniqeContributors--;
        uint256 _dump = contributorValue[msg.sender] -
            ((10 * contributorValue[msg.sender]) / 100);
    ...
    function claim() external presaleEnded {
        ...
        if (contributionCount >= softcap) {
            for (uint256 i = 0; i < contributor[msg.sender]; i++) {
                mint(msg.sender);
            }
        } else {
    ...
    function mint(address to) internal {
        require(
            maxSupply > totalSupply() + platformToken,
            "tokens are all claimed"
        );
        _safeMint(to, nonce);
        nonce++;
    }
    ...
    function withdraw() public onlyOwner presaleEnded {
        ...
        if (contribution >= softcap) {
            uint256 _profit = (platformCut / 100) * contribution;
            uint256 profit = address(this).balance - _profit;
            contribution = 0;
            platformCut = 0;
            if (platformToken > 0) {
